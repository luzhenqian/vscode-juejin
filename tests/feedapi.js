...
const getPostListCateConfig = {
  url: "https://api.juejin.cn/recommend_api/v1/article/recommend_cate_feed?aid=2608&uuid=7105256514437449255",
  method: "POST",
  headers: {
...
async function get() {
  const res = await axios(getPostListCateConfig)
  const data = res.data.data.map(item => item.article_id)
  console.log(data)
}
