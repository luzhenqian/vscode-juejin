...
export function Input(props: InputProps) {
  const { onSearch, onClose, placeholder } = props;
  React.useEffect(() => {
  ...
  return (
    <div className="flex items-center justify-center">
      <svg
        className="w-8 h-8 m-1 fill-gray-400 dark:fill-gray-300"
      ...
      <span onClick={onClose}>
        <Close className="w-8 h-8 mx-2 fill-gray-400 dark:fill-gray-300" />
      </span>
    </div>
  );
}
